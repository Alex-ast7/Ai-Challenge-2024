...
from .GraphDeberta import DebertaGraphV2Model
...
class CustomMultipleChoiceEncoder(nn.Module):
    ...
    def forward(
        ...
        token_type_ids = {'token_type_ids':flat_token_type_ids} if self.cfg.use_tkds else {}
        outputs = self.model(
            flat_input_ids,
        ...
        if self.cfg.pool == 'last_token':
            pooled_output = self.pool(flat_input_ids, outputs[0])
...
class CustomGraphMultipleChoiceEncoder(nn.Module):
    def __init__(self,cfg):
        super().__init__()
        self.cfg = cfg
        self.config = AutoConfig.from_pretrained(cfg.model)
        ...
        self.model = DebertaGraphV2Model.from_pretrained(cfg.model,config=self.config)
        if cfg.use_lora:
            peft_config = LoraConfig(
                r=cfg.lora.r,
                lora_alpha=cfg.lora.lora_alpha,
    ...
    def forward(
        ...
        inputs_embeds: Optional[torch.Tensor] = None,
        labels: Optional[torch.Tensor] = None,
        graph_encodes: Optional[torch.Tensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, MultipleChoiceModelOutput]:
        ...
        graph_attention_mask = torch.cat([
            flat_attention_mask,torch.ones(flat_attention_mask.shape[0],gnn_embeds.shape[1]).to(attention_mask.device)
        ],dim=1)
        ...
        if self.cfg.pool == 'last_token':
            ...
            pooled_output = self.pool(outputs[0],graph_attention_mask)
        else:
            pooled_output = self.pool(outputs, graph_attention_mask)
...
class CustomGraphMLMEncoder(nn.Module):
    def __init__(self,cfg):
        super().__init__()
        self.cfg = cfg
        self.config = AutoConfig.from_pretrained(cfg.model)
    ...
    def forward(
        ...
        flat_input_ids = input_ids.view(-1, input_ids.size(-1)) if input_ids is not None else None
        flat_labels = labels.view(-1, labels.size(-1)) if labels is not None else None
        flat_position_ids = position_ids.view(-1, position_ids.size(-1)) if position_ids is not None else None
        flat_token_type_ids = token_type_ids.view(-1, token_type_ids.size(-1)) if token_type_ids is not None else None
        flat_attention_mask = attention_mask.view(-1, attention_mask.size(-1)) if attention_mask is not None else None
        flat_inputs_embeds = ( 
        ...
        logits = self.fc(outputs[0][:, :-64])

        loss = None
        if labels is not None:
            loss_fct = nn.CrossEntropyLoss(label_smoothing=self.cfg.label_smoothing)
            loss = loss_fct(logits.view(-1, self.config.vocab_size), flat_labels.view(-1))
